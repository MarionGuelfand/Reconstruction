...
int main(int argc, char* argv[])
{
  ...
  if (recons_type==0) {
    ...
    file_out_cherenkov_angle << text_dir << "/Rec_adf_cherenkov_angle.txt";
    fBox.fitModel()  = ADF;
  }
  else {
  ...
  fid_out_cherenkov_angle = fopen( file_out_cherenkov_angle.str().c_str(), "w" );
  ...
  double GroundAltitude = 1264.;//1086.;//2900.; 
  ...
  if (recons_type==2){
    ...
    fscanf( fid_input_xmax, "%ld %ld %lf %lf %lf %lf %lf %lf %lf",&iCoinc_rec, &dummy_int, &dummy, &dummy, &x_xmax ,&y_xmax, &z_xmax, &dummy, &dummy); // read recons of Xmax from spherical recons for the first coinc !!!
  }
      ...
      if (iCoinc_prev != iCoinc || i==0 )  {  // New coinc
        ...
        if (fBox.Na()>3){
          ...
          if (recons_type==1 || recons_type==2){
            ...
            fBox.theta_input() = 180-theta_rec;
            fBox.phi_input() = fmod(180+phi_rec,360);
          ...
          if (recons_type==0) {	// Plan wave
            fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.8le %12.5le %12.5le \n", iCoinc_prev, fBox.Na(), 180-fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fmod(180+fBox.phi()*RAD2DEG,360), fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance()); // plan wave parameters
          }
          else if (recons_type==1){ // Spherical wave
              fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())), GroundAltitude - fBox.r_xmax()*cos(fBox.theta()), fBox.r_xmax(), fBox.ts(), fBox.theta()*RAD2DEG, fBox.phi()*RAD2DEG); // Physical spherical wave parameters
              std::cout<<"x = "<<- fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi()))<<" y = "<<- fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi()))<<" z = "<< GroundAltitude - fBox.r_xmax()*cos(fBox.theta())<<std::endl;
            }
            else if (recons_type==2){ //ADF
                fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le \n", iCoinc_prev, fBox.Na(), 180-fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fmod(180+fBox.phi()*RAD2DEG,360), fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance(), fBox.cerenkov_width(), fBox.amp_corr()); // ADF parameters
                for (size_t i =0; i < fBox.Cherenkov_angle_vector().size(); i++) {
                  fprintf(fid_out_cherenkov_angle, "%ld %12.10le %12.5le %12.5le %12.10le\n", iCoinc_prev, fBox.Cherenkov_angle_vector()[i]*RAD2DEG, fBox.eta_vector()[i]*RAD2DEG, fBox.adf_vector()[i], fBox.omega_vector()[i]*RAD2DEG);
                }
              }
            //------
            if (recons_type==2){
              fscanf( fid_input_xmax, "%ld %ld %lf %lf %lf %lf %lf %lf %lf",&iCoinc_rec, &dummy_int, &dummy, &dummy, &x_xmax ,&y_xmax, &z_xmax, &dummy, &dummy); // read recons of Xmax from spherical recons for the next coinc !!
              if (iCoinc_rec != iCoinc_prev){std::cout<<"Warning wrong input coinc event"<<std::endl;}
  ...
  //if ( fBox.Na()>3  && (fBox.ta(2)+fBox.ta(3)>0 ) ) {  // Timing performed for intercorrelation
    ...
    if (recons_type==0)  {   // Plan wave
      ...
      fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())), GroundAltitude - fBox.r_xmax()*cos(fBox.theta()), fBox.r_xmax(), fBox.ts(), fBox.theta()*RAD2DEG, fBox.phi()*RAD2DEG); // Physical Hyperbolic wave parameters
    }
    else if (recons_type==2){ //ADF
        fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), 180-fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fmod(180+fBox.phi()*RAD2DEG,360), fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance(), fBox.cerenkov_width(), fBox.amp_corr()); // ADF parameters
        for (size_t i =0; i < fBox.Cherenkov_angle_vector().size(); i++) {
  ...
  if (recons_type==2){
  fclose( fid_out_cherenkov_angle );
  }
