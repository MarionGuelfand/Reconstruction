...
int main(int argc, char* argv[])
{
        ...
        if (fBox.Na()>3){
          ...
          if (recons_type==0) {	// Plan wave
            fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.8le %12.5le %12.5le \n", iCoinc_prev, fBox.Na(), 180-fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fmod(180+fBox.phi()*RAD2DEG,360), fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance()); // plan wave parameters
              ...
              fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())), GroundAltitude - fBox.r_xmax()*cos(fBox.theta()), fBox.r_xmax(), fBox.ts()); // Physical spherical wave parameters
  ...
  //if ( fBox.Na()>3  && (fBox.ta(2)+fBox.ta(3)>0 ) ) {  // Timing performed for intercorrelation
    ...
    if (recons_type==0)  {   // Plan wave
      ...
      fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())), GroundAltitude - fBox.r_xmax()*cos(fBox.theta()), fBox.r_xmax(), fBox.ts()); // Physical Hyperbolic wave parameters
    }
    else if (recons_type==2){ //ADF
        fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), 180-fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fmod(180+fBox.phi()*RAD2DEG,360), fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance(), fBox.cerenkov_width(), fBox.amp_corr()); // ADF parameters
        for (size_t i =0; i < fBox.Cherenkov_angle_vector().size(); i++) {
