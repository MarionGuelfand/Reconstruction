...
#define MAX_R 1e10
{
  ...
  else if (_fitModel == SPHERE_WAVE)
  {
    ...
    //for background
    _xPORT.B( 1, 1 ) = (0.)*M_PI/180;  // theta parameter (1st) min value (1)
    _xPORT.B( 2, 1 ) = (180.)*M_PI/180;  // theta parameter (1st) max value (2)
    _xPORT.B( 1, 2 ) = (0.)*M_PI/180;    // phi parameter (2nd) min value (1)
    _xPORT.B( 2, 2 ) = (360.)*M_PI/180;    // phi parameter (2nd) max value (2)
    _xPORT.B( 1, 3) = 0;    // r parameter (3th) min value (1)
    _xPORT.B( 2, 3) = 20000;     // r parameter (3th) max value (2)
    _xPORT.B( 1, 4) = -20000;  // t parameter (3th) max value (2) similar to r !
    _xPORT.B( 2, 4) = 0;
                                                  // t parameter (3th) min value (1) similar to r !
    // //Debug
    // _xPORT.B( 1, 1 ) = 92.92*M_PI/180.;  // theta parameter (1st) min value (1)
    // _xPORT.B( 2, 1 ) = 92.92*M_PI/180.;  // theta parameter (1st) max value (2)
...
//Cerenkov pattern fit based on amplitudes measurments
{
    ...
    for ( int i = 0; i < Na[ 0 ]; i++)
  {
      ...
      //store eta values and Cherenkov angle values
      Xa[ i*N_ANTENNA_DATA + 8 ] = _cerenkov;
      Xa[ i*N_ANTENNA_DATA + 9 ] = _eta;
      Xa[ i*N_ANTENNA_DATA + 10 ] = _adf;
      Xa[ i*N_ANTENNA_DATA + 11 ] = _omega;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void FitTools::Cherenkov_value()
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
{
  _Cherenkov_angle_vector.resize(0);
  _eta_vector.resize(0);
  _adf_vector.resize(0);
  _omega_vector.resize(0);
  for (int i=0; i<this->Na(); i++) {
    _Cherenkov_angle_vector.push_back(_Xa[ i*N_ANTENNA_DATA + 8 ]);
    _eta_vector.push_back(_Xa[ i*N_ANTENNA_DATA + 9 ]);
    _adf_vector.push_back(_Xa[ i*N_ANTENNA_DATA + 10 ]);
    _omega_vector.push_back(_Xa[ i*N_ANTENNA_DATA + 11 ]);
  }
}