...
#define MAX_R 1e10
{
  ...
  if ( _fitModel == PLAN_WAVE )
  {
    ...
    //===
    _xPORT.B( 1, 1 ) = 90.*M_PI/180;    // theta parameter (1st) min value (1) is 0.0
  ...
  else if (_fitModel == SPHERE_WAVE)
  {
    ...
    _xPORT.B( 1, 1 ) = -1.e4; // X parameter (1st) min value (1)
    _xPORT.B( 2, 1 ) = 1.e4;  // X parameter (1st) max value (2)
    _xPORT.B( 1, 2 ) = -1.e4; // Y parameter (2nd) min value (1)
    _xPORT.B( 2, 2 ) = 1.e4;  // Y parameter (2nd) max value (2)
    _xPORT.B( 1, 3) = 0.;     // Z parameter (3th) min value (1)
    _xPORT.B( 2, 3) = 1.e4;   // Z parameter (3th) max value (2)
    _xPORT.B( 1, 4) = -1.e4;  // t parameter (3th) max value (2) similar to r !
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
{
  ...
  else if ( _fitModel == SPHERE_WAVE )
  {
    // Get fit parameters
    //===
    _x_source= _xPORT.X( 1 );
    _y_source= _xPORT.X( 2 );
    _z_source= _xPORT.X( 3 );
    _ts    = _xPORT.X( 4 );
    _cr    = _xPORT.X( 5 );
  }
  else if ( _fitModel == ADF )
...

{
  ...
  double x_v[N_X];
  for (int i=0; i< N_X; i++){
    x_v[i] = (_xPORT.B( 1, 1 ) + (_xPORT.B( 2, 1 ) - _xPORT.B( 1, 1 ))/(N_X-1)*i)*180./M_PI;
    //std::cout<<"x_v[i] = "<<x_v[i]<<std::endl;
  }

  double y_v[N_Y];
  for (int i=0; i< N_Y; i++){
    y_v[i] = (_xPORT.B( 1, 2 ) + (_xPORT.B( 2, 2 ) - _xPORT.B( 1, 2 ))/(N_Y-1)*i)*180./M_PI;
    //std::cout<<"y_v[i] = "<<y_v[i]<<std::endl;
  }

  double z_v[N_Z];
  for (int i=0; i< N_Z; i++){
    z_v[i] = (_xPORT.B( 1, 3 ) + (_xPORT.B( 2, 3 ) - _xPORT.B( 1, 3 ))/(N_Z-1)*i);
    //std::cout<<"z_v[i] = "<<z_v[i]<<std::endl;
  }
  double ts_v[N_s];
  for (int i=0; i< N_s; i++){
  ...
  for ( int ic = 0; ic < ic_max; ic++ )
    for ( int it = 0; it < N_X; it++ )
      for ( int ip = 0; ip < N_Y; ip++ )
        for (int ir =0; ir < N_Z; ir++ )
              ...
              for (int iamp_corr = 0; iamp_corr < Namp_corr; iamp_corr++ )
  {
    ...
    if ( _fitModel == PLAN_WAVE )
    {
      this->theta() = x_v[ it ]*deg2rad;
      this->phi()   = y_v[ ip ]*deg2rad;
    }
    else if ( _fitModel == SPHERE_WAVE )
    {
      this->x_source() = x_v[ it ]*deg2rad;
      this->y_source()   = y_v[ ip ]*deg2rad;
      this->z_source() = z_v[ ir ];
      this->ts()   = ts_v[ is ];
    }

    else if ( _fitModel == ADF )
    {
      this->theta() = 0.;//modif //theta_v[ it ]*deg2rad;
      this->phi()   = 0.;//modif //phi_v[ ip ]*deg2rad;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
{
  ...
  y_xmax =  X[ 1 ];
  z_xmax =  X[ 2 ];
  //std::cout<<"x_xamx = "<<x_xmax<<std::endl;
    ...
    for ( int i = 0; i < Na[ 0 ]; i++)
  {
    ...
    // std::cout<<"F[ 0 ]  = "<<F[ 0 ]<<" X[ 0 ] = "<<X[ 0 ]<<" X[ 1 ] = "<<X[ 1 ]<<" X[ 2 ] = "<<X[ 2 ]<<" X[ 3 ] = "<<X[ 3 ]<<" X[ 4 ] = "<<X[ 4 ]<<std::endl;
}
